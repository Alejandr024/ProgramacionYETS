...
public class OrganizacionAsesinos {
    ...
    public Asesino getAsesino(int indice) {
        if (indice < 0 || indice >= arrayAsesinos.length) {
            return null;
        }
    ...
    public void backUp(Asesino[] asesinos) {

        try (BufferedWriter out = new BufferedWriter(new FileWriter("organizacion_backup.txt", true));) {

            for (int i = 0; i < asesinos.length; i++) {
                String linea = asesinos[i].getNombre() + "::" + asesinos[i].getInteligencia() + "::" + asesinos[i].getSuerte() + "::" + asesinos[i].isIsLicencia();
                out.write(linea);
                out.newLine();
            }
        ...
        System.out.println("Los asesinos se han agregado al fichero \"organizacion_backup.txt\" con exito.");
    }
